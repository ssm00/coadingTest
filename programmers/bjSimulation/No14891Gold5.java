...
public class No14891Gold5 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < rotateNum; i++) {
            int status[] = Arrays.stream(br.readLine().split(" ")).mapToInt(x -> Integer.parseInt(x)).toArray();
        ...
        int top1 = gearSets[0].status[0];
        int top2 = gearSets[1].status[0];
        int top3 = gearSets[2].status[0];
        int top4 = gearSets[3].status[0];
        int point = 0;
        if (top1 == 1) {
            point += 1;
        }
